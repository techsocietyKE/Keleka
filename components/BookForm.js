    ...
    return (
        <form onSubmit={saveProduct} className="p-4 bg-white rounded-lg shadow-lg max-w-2xl mx-auto">
            ...
            <input
                ...
                value={searchQuery}
                onChange={(ev) => setSearchQuery(ev.target.value)} // Update search query on input change
                className="w-full border border-gray-300 rounded-lg p-2 mb-2 outline-none focus:border-blue-500"
            />
            <select
                className="w-full border border-gray-300 rounded-lg p-2 mb-4 outline-none focus:border-blue-500"
                value={genre}
                onChange={(ev) => setGenre(ev.target.value)} // Update selected genre on change
            >
            ...
            <div className="flex flex-wrap gap-4 mb-4">
                {!!images.length && images.map((uploadedImage, index) => (
                    <div key={index} className="h-24 bg-white p-4 shadow-sm rounded-sm border border-gray-200">
                        <img src={uploadedImage} alt={`Uploaded Image ${index}`} className="rounded-lg w-full h-full object-cover" />
                    </div>
                ))}
            ...
            <button type="submit" className="w-full bg-blue-600 text-white rounded-lg py-2 mt-4 hover:bg-blue-700 transition">
                Save
            </button>
        </form>
    );
